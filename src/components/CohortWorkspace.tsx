...
import {
  ...
  getExampleCohortProject,
  getNextCohortStage,
  getPrevCohortStage,
  getCohortProject,
  getCohortStage,
  markCohortStageComplete,
  saveCohortStageData,
  type CohortProject,
  type CohortStageId,
} from "@/lib/cohort";
...
export function CohortWorkspace({
  ...
  const stage = getCohortStage(stageId);
  ...
  const load = useCallback(() => {
    if (readOnly && projectId === "example-cohort") {
      const ex = getExampleCohortProject();
      setProject(ex);
      applyStage(ex);
      return;
    }
    const p = getCohortProject(projectId);
    if (!p) {
  ...
  function applyStage(p: CohortProject) {
  ...
  const progress = useMemo(
    () =>
      project
        ? computeCohortProgress(project)
        : { percent: 0, completed: 0, total: COHORT_STAGE_ORDER.length },
    [project]
  );

  const prev = getPrevCohortStage(stageId);
  const next = getNextCohortStage(stageId);
  const base = readOnly
    ? "/designs/cohort/example"
    : `/designs/cohort/${projectId}`;

  async function persist(
    extraData?: Record<string, unknown>,
    extras?: Parameters<typeof saveCohortStageData>[3]
  ) {
    ...
    try {
      ...
      const updated = saveCohortStageData(projectId, stageId, data, extras);
  ...
  return (
    <div className="mx-auto max-w-3xl px-3 py-6 sm:px-6 sm:py-8">
      ...
      <div className="mt-4 -mx-1 flex gap-1.5 overflow-x-auto px-1 pb-2">
        {COHORT_STAGE_ORDER.map((id) => {
          const s = getCohortStage(id);
                    ...
                    onChange={(e) => {
                      ...
                      void (async () => {
                        ...
                        try {
                          ...
                          const updated = saveCohortStageData(
                            projectId,
                            stageId,
                            data
                          );
            ...
            {stageId === "reporting" && (
              <div className="border-t border-slate-100 pt-4">
                <h3 className="text-sm font-semibold text-slate-900">
                  STROBE cohort-inspired checklist
                </h3>
                <ul className="mt-3 space-y-2">
                  {STROBE_COHORT_ITEMS.map((item) => {
              ...
              <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    markCohortStageComplete(projectId, stageId);
                    load();
                  }}
                ...
                <button
                  ...
                  onClick={() =>
                    downloadText(
                      ...
                      buildCohortExportMarkdown(getCohortProject(projectId) || project)
                    )
                  }
