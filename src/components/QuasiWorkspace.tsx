...
import {
  ...
  computeQuasiProgress,
  getExampleQuasiProject,
  getNextQuasiStage,
  getPrevQuasiStage,
  getQuasiProject,
  getQuasiStage,
  markQuasiStageComplete,
  saveQuasiStageData,
  type QuasiProject,
  type QuasiStageId,
} from "@/lib/quasi";
...
export function QuasiWorkspace({
  ...
  const stage = getQuasiStage(stageId);
  const [project, setProject] = useState<QuasiProject | null>(null);
  ...
  const load = useCallback(() => {
    if (readOnly && projectId === "example-quasi") {
      const ex = getExampleQuasiProject();
      setProject(ex);
      applyStage(ex);
      return;
    }
    const p = getQuasiProject(projectId);
    if (!p) {
  ...
  function applyStage(p: QuasiProject) {
  ...
  const progress = useMemo(
    () =>
      project
        ? computeQuasiProgress(project)
        : { percent: 0, completed: 0, total: QUASI_STAGE_ORDER.length },
    [project]
  );

  const prev = getPrevQuasiStage(stageId);
  const next = getNextQuasiStage(stageId);
  const base = readOnly
    ? "/designs/quasi/example"
    : `/designs/quasi/${projectId}`;

  async function persist(
    extraData?: Record<string, unknown>,
    extras?: Parameters<typeof saveQuasiStageData>[3]
  ) {
    ...
    try {
      ...
      if (stageId === "reporting") data._trendChecks = checks;
      if (extraData) Object.assign(data, extraData);
      const updated = saveQuasiStageData(projectId, stageId, data, extras);
  ...
  if (error && !project) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center text-slate-600">
        {error}.{" "}
        <Link href="/designs/quasi" className="text-teal-700 underline">
          Back
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-3xl px-3 py-6 sm:px-6 sm:py-8">
      ...
      <div className="mt-4 -mx-1 flex gap-1.5 overflow-x-auto px-1 pb-2">
        {QUASI_STAGE_ORDER.map((id) => {
          const s = getQuasiStage(id);
                      ...
                      void (async () => {
                        ...
                        try {
                          ...
                          if (stageId === "reporting")
                            data._trendChecks = checks;
                          const updated = saveQuasiStageData(
                            projectId,
                            stageId,
                            data
                          );
            ...
            {stageId === "reporting" && (
              <div className="border-t border-slate-100 pt-4">
                <h3 className="text-sm font-semibold text-slate-900">
                  TREND / quasi checklist
                </h3>
                <ul className="mt-3 space-y-2">
                  {TREND_QUASI_ITEMS.map((item) => {
                        ...
                        <label className="flex gap-2 text-sm text-slate-700">
                          <input
                            ...
                            onChange={() => {
                              ...
                              void persist({ _trendChecks: nextChecks });
                            }}
                            className="mt-0.5"
                          />
                          <span>
              ...
              <div className="flex flex-wrap gap-2 border-t border-slate-100 pt-4">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    markQuasiStageComplete(projectId, stageId);
                    load();
                  }}
