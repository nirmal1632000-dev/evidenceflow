...
export default function CaseProjectHomePage() {
  ...
  const progress = computeCaseProgress(project);
  const base = `/designs/case-report/${id}`;
  ...
  return (
    <div className="mx-auto max-w-2xl px-3 py-8 sm:px-6">
      ...
      <p className="mt-1 text-sm text-slate-500">
        {project.mode === "series" ? "Case series" : "Case report"} ·{" "}
      ...
      <div className="mt-6 flex flex-col gap-2 sm:flex-row">
        ...
        <button
          ...
          onClick={() => {
            const md = buildCaseExportMarkdown(project);
      ...
      <ul className="mt-8 divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
        {CASE_STAGE_ORDER.map((sid) => {
          const s = getCaseStage(sid);
