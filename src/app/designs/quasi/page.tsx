...
export default function QuasiIndexPage() {
  const [projects, setProjects] = useState<QuasiProject[]>([]);
  const [title, setTitle] = useState("");

  useEffect(() => {
    setProjects(listQuasiProjects());
  }, []);
  ...
  function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    const p = createQuasiProject(title || "My quasi-experimental evaluation");
    window.location.href = `/designs/quasi/${p.id}/question`;
  }
      ...
      >
        ...
        <label className="block text-sm">
          ...
          <input
            ...
            placeholder="e.g. Hand-hygiene campaign evaluation"
      ...
      <section className="mt-10">
        ...
        {projects.length === 0 ? (
          ...
          <ul className="mt-3 space-y-3">
            {projects.map((p) => {
              const prog = computeQuasiProgress(p);
              const stage = getQuasiStage(p.currentStage);
              return (
                <li
                  key={p.id}
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <div className="flex gap-2">
                      <Link
                        href={`/designs/quasi/${p.id}/${p.currentStage}`}
