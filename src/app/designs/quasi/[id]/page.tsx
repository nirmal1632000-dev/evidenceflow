...
export default function QuasiProjectHomePage() {
  ...
  const progress = computeQuasiProgress(project);
  const base = `/designs/quasi/${id}`;

  return (
    <div className="mx-auto max-w-2xl px-3 py-8 sm:px-6">
      <Link href="/designs/quasi" className="text-sm text-teal-700 underline">
        ← All quasi projects
      </Link>
      ...
      <p className="mt-1 text-sm text-slate-500">
        Quasi-experimental · {progress.completed}/{progress.total} stages
        complete
      </p>
      ...
      <ul className="mt-8 divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
        {QUASI_STAGE_ORDER.map((sid) => {
          const s = getQuasiStage(sid);
