...
export default function XsProjectHomePage() {
  ...
  const progress = computeXsProgress(project);
  const base = `/designs/cross-sectional/${id}`;
  ...
  return (
    <div className="mx-auto max-w-2xl px-3 py-8 sm:px-6">
      ...
      >
        ← All cross-sectional projects
      </Link>
      <h1 className="mt-3 text-2xl font-semibold text-slate-900">
        {project.title}
      </h1>
      <p className="mt-1 text-sm text-slate-500">
        Cross-sectional · {progress.completed}/{progress.total} stages complete
      </p>
      ...
      <div className="mt-6 flex flex-col gap-2 sm:flex-row">
        ...
        <button
          ...
          onClick={() => {
            const md = buildXsExportMarkdown(project);
      ...
      <ul className="mt-8 divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
        {XS_STAGE_ORDER.map((sid) => {
          const s = getXsStage(sid);
