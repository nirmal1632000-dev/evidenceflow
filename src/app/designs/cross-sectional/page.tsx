...
export default function CrossSectionalIndexPage() {
  const [projects, setProjects] = useState<XsProject[]>([]);
  const [title, setTitle] = useState("");

  useEffect(() => {
    setProjects(listXsProjects());
  }, []);
  ...
  function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    const p = createXsProject(title || "My cross-sectional study");
    window.location.href = `/designs/cross-sectional/${p.id}/question`;
  }
      ...
      >
        ...
        <label className="block text-sm">
          ...
          <input
            ...
            placeholder="e.g. Sleep and anxiety among first-year students"
      ...
      <section className="mt-10">
        ...
        {projects.length === 0 ? (
          ...
          <ul className="mt-3 space-y-3">
            {projects.map((p) => {
              const prog = computeXsProgress(p);
              const stage = getXsStage(p.currentStage);
              return (
                <li
                  key={p.id}
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    <div className="min-w-0">
                      <Link
                        href={`/designs/cross-sectional/${p.id}`}
                    ...
                    <div className="flex gap-2">
                      <Link
                        href={`/designs/cross-sectional/${p.id}/${p.currentStage}`}
